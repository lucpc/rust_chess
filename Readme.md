...
*   **Movimentos Especiais:** Implementa regras especiais como Roque (*Castling*) e *En Passant*.
...
1.  **Clone o Repositório** (ou simplesmente descompacte os arquivos em uma pasta):
    ```bash
    # Se estivesse no GitHub, seria assim:
    # git clone https://github.com/seu-usuario/rust-chess.git
    ```
...
Target: _
```
*(Neste exemplo, imagine que os movimentos possíveis da peça `e2` estão com fundo azul)*
...
*   **En Passant:** A captura *en passant* é realizada movendo seu peão na diagonal para a casa vazia atrás do peão adversário que acabou de avançar duas casas.

...
impl ChessMatch {
    ...
    pub fn perform_chess_move(
        ...
        if let Some(moved_piece_at_target) = self.board.piece(target_pos) {
            let piece_display = moved_piece_at_target.to_string();
    ...
    fn test_check(&self, color: Color) -> bool {
        ...
        let opponent_pieces = self
            .pieces_on_board
            .iter()
            .filter(|&&pos| self.board.piece(pos).unwrap().color() == opponent)
            .cloned()
            .collect::<Vec<_>>();

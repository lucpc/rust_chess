...
impl Board {
    pub fn new(rows: usize, cols: usize) -> Result<Self, ChessError> {
        if rows < 1 || cols < 1 {
            return Err(ChessError(
                "Error creating board: there must be at least 1 row and 1 column".to_string(),
            ));
        }
    ...
    pub fn piece(&self, position: Position) -> Option<&Box<dyn Piece>> {
        if !self.position_exists(position) {
            panic!("Position is not on the board: {}", position);
        }
    ...
    pub fn place_piece(
        ...
        if !self.position_exists(position) {
            return Err(ChessError(format!(
                "Cannot place piece: invalid position {}",
                position
            )));
        }
        ...
        if self.there_is_a_piece(position) {
            return Err(ChessError(format!(
                "There is already a piece on position {}",
                position
            )));
        }
    ...
    pub fn remove_piece(
        ...
        if !self.position_exists(position) {
            panic!("Cannot remove: invalid position {}", position);
        }
